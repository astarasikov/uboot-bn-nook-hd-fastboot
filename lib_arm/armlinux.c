...
static void setup_cpuid_tag(struct tag **in_params);
...
{
	u8 * cpuid = NULL;
	params->hdr.tag = ATAG_CPUID;
	cpuid = & (params->u.cpuid.id);
	SEC_ENTRY_Std_Ppa_Call ( PPA_SERV_HAL_BN_CPUID , 1 , cpuid );
	params->hdr.size = tag_size (tag_cpuid);
	params = tag_next (params);
	printf("u-boot:cpuid:%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
		cpuid[0], cpuid[1], cpuid[2], cpuid[3], cpuid[4],
		cpuid[5], cpuid[6], cpuid[7], cpuid[8], cpuid[9],
		cpuid[10], cpuid[11], cpuid[12], cpuid[13], cpuid[14],
		cpuid[15], cpuid[16], cpuid[17], cpuid[18], cpuid[19]);
}
