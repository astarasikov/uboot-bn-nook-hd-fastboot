...
/* TODO to be removed when dynamic DRAM size detection is fixed */
...
int dram_init(void)
{
	...
	/*
	 * Revert mapping changes done in x-loader
	 * before calculating dram size
	 * TODO: This two lines should be removed when the changes in
	 * x-loader are reverted
	 */

	__raw_writel(0x80640300, DMM_BASE + DMM_LISA_MAP_0);
	__raw_writel(0x00000000, DMM_BASE + DMM_LISA_MAP_1);
	...
	gd->bd->bi_dram[0].size = sdram_size_hwid();
