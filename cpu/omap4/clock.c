...
#define OMAP4470_VDD_MPU_OPPNITROSB_UV_MASK	0x36
#define OMAP4470_VDD_MPU_OPPNITRO_UV_MASK	0x35
#define OMAP4470_VDD_MPU_OPPTURBO_UV_MASK	0x31
#define OMAP4470_VDD_MPU_OPP100_UV_MASK		0x28
#define OMAP4470_VDD_MPU_OPP50_UV_MASK		0x1a
...
enum mpu_dpll {
	OMAP4470_VDD_MPU_OPP50,
	OMAP4470_VDD_MPU_OPP100,
	OMAP4470_VDD_MPU_OPPTURBO,
	OMAP4470_VDD_MPU_OPPNITRO,
	OMAP4470_VDD_MPU_OPPNITROSB,
};

/* Tables having M, N, M2 values for different sys_clk speeds
 * The tables are organized as follows:
 * Rows : 1 - 12M, 2 - 13M, 3 - 16.8M, 4 - 19.2M, 5 - 26M, 6 - 27M, 7 - 38.4M
 */

/* MPU parameters */
struct dpll_param mpu_dpll_param_1500mhz[7] = {
	/* 12M values */
...
struct dpll_param mpu_dpll_param_1300mhz[7] = {
	/* 12M values */
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	/* 13M values */
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	...
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	/* 38.4M values */
	{0xa9, 0x09, 0x0, 0x00, 0x00, 0x00, 0x00},
};

struct dpll_param mpu_dpll_param_1100mhz[7] = {
	/* 12M values */
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	/* 13M values */
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	...
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	/* 38.4M values */
	{0x2b, 0x2, 0x0, 0x00, 0x00, 0x00, 0x00},
};
...
struct dpll_param mpu_dpll_param_800mhz[7] = {
	...
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	/* 38.4M values */
	{0x7d, 0x5, 0x1, 0x00, 0x00, 0x00, 0x00},
};

struct dpll_param mpu_dpll_param_400mhz[7] = {
	/* 12M values */
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	/* 13M values */
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	...
	{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
	/* 38.4M values */
	{0x7d, 0x5, 0x2, 0x00, 0x00, 0x00, 0x00},
};
...
struct dpll_param mpu_dpll_param[7] = {
	...
	{0x1a, 0x2, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0},
#else
	{0x7d, 0x7, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0},
#endif
};
...
static int omap_vc_bypass_send_value(u8 sa, u8 reg_addr, u8 reg_data)
{
	...
	__raw_writel(reg_val | PRM_VC_VAL_BYPASS_VALID_BIT,
			PRM_VC_VAL_BYPASS);
...
static int program_mpu_dpll(const mpu_dpll, const u32 clk_index,
				dpll_param **dpll_param_p)
{
	u8 mask;

	/* enable DCC for freq higher than or equal to 1Ghz */
	sr32(CM_CLKSEL_DPLL_MPU, 22, 1, (mpu_dpll != OMAP4470_VDD_MPU_OPP50)
					&& (mpu_dpll != OMAP4470_VDD_MPU_OPP100));

	switch(mpu_dpll) {
		case OMAP4470_VDD_MPU_OPP50:
			*dpll_param_p = &mpu_dpll_param_400mhz[clk_index];
			mask = OMAP4470_VDD_MPU_OPP50_UV_MASK;
			break;
		case OMAP4470_VDD_MPU_OPP100:
		default:
			*dpll_param_p = &mpu_dpll_param_800mhz[clk_index];
			mask = OMAP4470_VDD_MPU_OPP100_UV_MASK;
			break;
		case OMAP4470_VDD_MPU_OPPTURBO:
			*dpll_param_p = &mpu_dpll_param_1100mhz[clk_index];
			mask = OMAP4470_VDD_MPU_OPPTURBO_UV_MASK;
			break;
		case OMAP4470_VDD_MPU_OPPNITRO:
			*dpll_param_p = &mpu_dpll_param_1300mhz[clk_index];
			mask = OMAP4470_VDD_MPU_OPPNITRO_UV_MASK;
			break;
		case OMAP4470_VDD_MPU_OPPNITROSB:
			*dpll_param_p = &mpu_dpll_param_1500mhz[clk_index];
			mask = OMAP4470_VDD_MPU_OPPNITROSB_UV_MASK;
			break;
	}

	return omap_vc_bypass_send_value(TWL6032_SRI2C_SLAVE_ADDR,
				TWL6032_SRI2C_REG_ADDR_SMPS1, mask);
}


static void configure_mpu_dpll_with_opp(u32 clk_index, const enum mpu_dpll new_opp)
{
	dpll_param *dpll_param_p;
	int r;
	...
	/* Program MPU DPLL */
	if(get_cpu_type() == CPU_4470) {
		r = program_mpu_dpll(new_opp, clk_index, &dpll_param_p);
		/* in case of error try to set mpu @ OPP 100*/
		if (r)
			dpll_param_p = &mpu_dpll_param_800mhz[clk_index];
	} else
...
{
	configure_mpu_dpll_with_opp(clk_index, OMAP4470_VDD_MPU_OPP100);
}
...
void set_mpu_dpll_min_opp(void)
{
	u32 clk_index;
	...
	configure_mpu_dpll_with_opp(clk_index - 1, OMAP4470_VDD_MPU_OPP50);
}
