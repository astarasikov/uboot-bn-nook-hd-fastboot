...
void data_lines_reset(unsigned int base) {

	ulong start = get_timer(0);

	OMAP_HSMMC_SYSCTL(base) |= SRD;

	while (!(OMAP_HSMMC_SYSCTL(base) & SRD))
		if (get_timer(start) > (MMC_TIMEOUT*CFG_HZ/1000))
			return;

	while (OMAP_HSMMC_SYSCTL(base) & SRD);
		if (get_timer(start) > (2*MMC_TIMEOUT*CFG_HZ/1000))
			return;
}
...
#define VMODE_ERROR (1 << 23)
	...
	case 0:
		/* SLOT-0 PBIAS prepare for power reset */
		value = CONTROL_PBIAS_LITE;
		if (value & VMODE_ERROR)
			printf("WARNING: voltage error, before MMC1 power reset\n");
		CONTROL_PBIAS_LITE = value & ~((1 << 26) || (1 << 22));

		/* Phoenix LDO config */
		/* turn off LDO */
		data = 0x20;
		i2c_write(0x48, 0x9A, 1, &data, 1);
		/* assign LDO to APP group */
		data = 0x01;
		i2c_write(0x48, 0x98, 1, &data, 1);
		/* configure LDO states */
		data = 0x03;
		i2c_write(0x48, 0x99, 1, &data, 1);
		/* configure 3V0 voltage */
		data = 0x15;
		...
		/* allow voltage to ramp up */
		udelay(1000);

		/* SLOT-0 PBIAS config */
		value = CONTROL_PBIAS_LITE;
		CONTROL_PBIAS_LITE = value | (1 << 26) | (1 << 22) | (1 << 21);
		/* 4 micro seconds for comparator to generate an error */
		udelay(4);
		value = CONTROL_PBIAS_LITE;
		if (value & VMODE_ERROR) {
			printf("ERROR: invalid voltage level on MMC1\n");
			/* on voltage error disable MMC1 IO */
			CONTROL_PBIAS_LITE = value & ~(1 << 22);
			return ret;
		}

		value = CONTROL_CONF_MMC1;
		value |= (1 << 31) | (1 << 30) | (1 << 26) | (1 << 25) | (1 << 24);
		value &= ~((1 << 29) | (1 << 28));
		CONTROL_CONF_MMC1 = value;
		break;
	case 1:
		break;
	default:
